                                ...
                                <div class="flex flex-col items-stretch justify-start gap-3">
                                    @if ($nextVideoTargetId)
                                        <button type="button" data-course-continue data-continue-target="#{{ $nextVideoTargetId }}" class="inline-flex items-center justify-center rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow hover:bg-blue-700">
                                            {{ $status === 'not_started' ? 'Начать обучение' : 'Продолжить' }}
                                ...
                                @if ($course->videos->isNotEmpty())
                                    <ul class="divide-y divide-gray-100 rounded-2xl border border-gray-100 bg-gray-50">
                                        @foreach ($course->videos as $video)
                                            ...
                                            @endphp
                                            <li id="{{ $videoElementId }}" class="video-item flex cursor-pointer items-stretch gap-4 px-5 py-4 transition hover:bg-white" data-course-id="{{ $course->id }}" data-course-title="{{ e($course->title) }}" data-video-id="{{ $video->id }}" data-video-title="{{ e($video->title) }}" data-video-url="{{ e($video->video_url) }}" data-video-preview-image="{{ e($video->preview_image ?? '') }}" data-video-short-description="{{ e($video->short_description ?? '') }}" data-video-full-description="{{ e($video->full_description ?? '') }}" data-access="allowed">
                                                <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-2xl border border-gray-200 bg-white text-sm font-semibold text-gray-500">
                                                    @if ($videoCompleted)
